...
- TEI: please refer to the [embedding](../embedding) for more information.
...
First, you need to install the `embedding` helm chart and one of the vector DB service, i.e. `redis-vector-db` chart.

After you've deployed dependency charts successfully, please run `kubectl get svc` to get the service endpoint URL respectively, i.e. `http://embedding:80`, `redis://redis-vector-db:6379`.
...
export TEI_EMBEDDING_ENDPOINT="http://embedding"
