...
- TEI: please refer to [embedding](../embedding) chart for more information
...
First, you need to get the dependent service deployed, i.e. deploy the `embedding` helm chart, `mm-embedding` helm chart, or contact prediction guard to get access info.

After you've deployed the dependent service successfully, please run `kubectl get svc` to get the backend service URL, e.g. `http://embedding`, `http://mm-embedding`.
...
export EMBEDDING_ENDPOINT="http://embedding"
