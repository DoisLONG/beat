...
helm install embedding embedding --set global.modelUseHostPath=${MODELDIR} --set EMBEDDING_MODEL_ID=${MODELNAME}
```

By default, the embedding service will downloading the "BAAI/bge-base-en-v1.5" which is about 1.1GB.
...
Then run the command `kubectl port-forward svc/embedding 2081:80` to expose the embedding service for access.
...
| global.modelUseHostPath | string | `"/mnt/opea-models"`                              | Cached models directory, embedding will not download if the model is cached here. The host path "modelUseHostPath" will be mounted to container as /data directory. Set this to null/empty will force it to download model. |
