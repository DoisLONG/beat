...
reranking-usvc depends on reranking, set the TEI_RERANKING_ENDPOINT as reranking endpoint.

## (Option1): Installing the chart separately

First, you need to install the reranking chart, please refer to the [reranking](../reranking) chart for more information.

After you've deployted the reranking chart successfully, please run `kubectl get svc` to get the tei service endpoint, i.e. `http://reranking`.

To install the reranking-usvc chart, run the following:

```console
cd GenAIInfra/helm-charts/common/reranking-usvc
export TEI_RERANKING_ENDPOINT="http://reranking"
...
cd GenAIInfra/helm-charts/common/reranking-usvc
helm dependency update
helm install reranking-usvc . --set reranking.enabled=true
```
