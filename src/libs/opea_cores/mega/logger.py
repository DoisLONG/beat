...
class CustomLogger:
    ...
    def __init__(self, name: str = None, level: str = "INFO"):
        ...
        for key, _level in log_config.items():
            logging.addLevelName(_level, key)
            if key == "EXCEPTION":
                self.__dict__[key.lower()] = self.logger.exception
            else:
                self.__dict__[key.lower()] = functools.partial(self.log_message, _level)
        ...
        if level.upper() not in log_config:
