...
logger = CustomLogger("comps-core-orchestrator", os.getenv("LOG_LEVEL", "INFO"))
...
class ServiceOrchestrator(DAG):
    ...
    async def schedule(self, initial_inputs: Dict, llm_parameters: LLMParams = LLMParams(), **kwargs):
        ...
        logger.debug(initial_inputs)

        # handle collection_name == 'SKIP'
        if hasattr(kwargs.get("retriever_parameters"), "collection_name") and kwargs[
            "retriever_parameters"].collection_name == "SKIP":
            logger.info("Currently not retriever, remove embedding retriever node")
            runtime_graph.delete_node_if_exists("embedding/MicroService")
            runtime_graph.delete_node_if_exists("retriever/MicroService")
            runtime_graph.delete_node_if_exists("rerank/MicroService")
            runtime_graph.add_edge("mcp/MicroService", "llm/MicroService")

        is_mcp = None
        if "is_mcp" in kwargs:
            is_mcp = kwargs["is_mcp"]
        # If search_type is not web_search, remove the web_search node and its related edges from the DAG
        if is_mcp is False and runtime_graph.has_node("mcp/MicroService"):
            logger.info("Currently not mcp, remove mcp node")
            runtime_graph.delete_node_if_exists("mcp/MicroService")
            logger.info(f"Start nodes after pruning: {runtime_graph.ind_nodes()}")
        ...
        async with aiohttp.ClientSession(trust_env=True, timeout=timeout) as session:
            pending = {
                ...
                for node in runtime_graph.ind_nodes()
            }
            ind_nodes = runtime_graph.ind_nodes()
                            ...
                            for downstream in reversed(downstreams):
                                try:
                                    if re.findall(black_node, downstream):
                                        logger.debug(f"skip forwardding to {downstream}...")
    ...
    async def execute(
        ...
        **kwargs,
        ...
        if self.services[cur_node].service_type == ServiceType.MCP:
            # prepare the mcp params
            mcp_parameters = kwargs.get("mcp_parameters", None)
            if mcp_parameters:
                inputs.update(mcp_parameters.dict())
